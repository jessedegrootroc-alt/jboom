...
(() => {
  ...
  window.__jboomVast?.abort();
  window.__jboomVast = new AbortController();
  const vast = { signal: window.__jboomVast.signal };
  ...
  const smoother = () => window.JBOOM?.smoother?.instantie || null;
  ...
  if (header) {
    ...
    const werkHeaderBij = () => {
      ...
      /* Heeft deze pagina geen lichte zone, dan hoort de balk hoe dan ook op
         donker te staan. Dat moet hier expliciet gebeuren en niet alleen in de
         HTML: de balk staat buiten de Barba-container en overleeft dus een
         pagina-overgang. Zonder deze regel hield hij na een sprong van de
         homepage naar bijvoorbeeld cookies.html zijn witte logo en witte
         menutekst, en die staan daar op een lichte ondergrond. */
      const licht = lichteZone ? nu < grens : false;
    ...
    if (gsapAanwezig && !kalm.matches) {
      ...
      window.__jboomVast.signal.addEventListener('abort',

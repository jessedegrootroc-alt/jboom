# -*- coding: utf-8 -*-
"""Gedeelde paginaschil voor de site van Aannemersbedrijf J. Boom.
...
pagina's identiek te houden terwijl ze gebouwd worden.
...
BASIS = "https://www.jboom.nl"

# Het hoofdmenu. Een item is of een gewone link, of een uitklapper met sublinks
# en een kaart ernaast.
#
# De sublinks komen uit SERVICES en LOSSE_DIENSTEN hieronder, zodat een
# wijziging in inhoud_jboom.py meteen in het menu, de voet en de
# overzichtspagina's landt. Daarom wordt NAV pas onderaan opgebouwd, in
# bouw_nav().
NAV = []

# De diensten in de vorm die de componenten verwachten:
# (bestand, titel, ondertitel, beeldsleutel).
SERVICES = [(b, t, C.dienst_kort(b), beeld) for b, t, beeld in D.HOOFDDIENSTEN]

# De werkzaamheden onder elke hoofddienst.
WERKZAAMHEDEN = {t: [(b, tt, C.dienst_kort(b), beeld)
                     for b, tt, _, beeld in D.kinderen(t)]
                 for _, t, _ in D.HOOFDDIENSTEN}

# De diensten die niet onder een hoofddienst hangen.
LOSSE_DIENSTEN = [(b, tt, C.dienst_kort(b), beeld)
                  for b, tt, _, beeld in D.LOSSE_DIENSTEN]

# Alle negen, in de volgorde van het menu: hoofddienst, dan zijn werkzaamheden.
ALLE_DIENSTEN = []
for _b, _t, _beeld in D.HOOFDDIENSTEN:
    ALLE_DIENSTEN.append((_b, _t, C.dienst_kort(_b), _beeld))
    ALLE_DIENSTEN += [(kb, kt, C.dienst_kort(kb), kbeeld)
                      for kb, kt, _, kbeeld in D.kinderen(_t)]
ALLE_DIENSTEN += LOSSE_DIENSTEN

PROJECTSERIES = D.PROJECTSERIES


def bouw_nav():
    """Het hoofdmenu.

       De bronsite heeft vijf items op de balk: Home, Diensten, Projecten, de
       twee projectcategorieen als subitems daaronder, Klanten over J. Boom en
       Contact. Die indeling is overgenomen; de vorm van het menu blijft die van
       dit template. Offerte staat er als eigen item bij, omdat de bron er op
       vrijwel elke pagina naar verwijst zonder dat het in het menu stond."""
    NAV.extend([
        {"soort": "link", "href": "index.html", "label": "Home"},
        {
            "soort": "uitklap", "id": "diensten", "label": "Diensten",
            "links": ([("diensten.html", "Alle diensten",
                        "Wat we doen, en welke dienst bij uw vraag past")]
                      + [(b, t, o) for b, t, o, _ in ALLE_DIENSTEN]),
            "kaart": {
                "kop": "Weet u nog niet wat u nodig heeft?",
                "tekst": "Vertel wat u wilt bereiken. In een vrijblijvend "
                         "orientatiegesprek kijken we wat er kan.",
                "knop": "Vraag een offerte aan",
                "href": "offerte.html",
                "foto": "jboom-aanbouw-16",
            },
        },
        {
            "soort": "uitklap", "id": "projecten", "label": "Projecten",
            "links": ([("projecten.html", "Alle projecten",
                        "Werk dat in en rondom Purmerend is opgeleverd")]
                      + [(s['bestand'], s['titel'],
                          f"{len(galerij(s['prefix']))} foto&rsquo;s")
                         for s in PROJECTSERIES]
                      + [("referenties.html", "Klanten over J. Boom",
                          f"{len(D.REFERENTIES)} referenties, ondertekend")]),
            "kaart": {
                "kop": "Klanten over J. Boom",
                "tekst": "Wat opdrachtgevers zelf over het werk schreven, met naam "
                         "en datum erbij.",
                "knop": "Lees de referenties",
                # Een foto met een gewone verhouding. De bandfoto's van 1920x480
                # die de bron voor de paginakoppen levert, verliezen in dit
                # staande vak zeventig procent van hun breedte.
                "href": "referenties.html",
                "foto": "jboom-dakkapel-05",
            },
        },
...
# Alle gegevens hieronder komen letterlijk van www.jboom.nl (de voet op elke
# pagina en de contactpagina). Er staat niets in dat de bronsite niet noemt.
TELEFOON_WEERGAVE = D.TELEFOON_WEERGAVE
TELEFOON_LINK = D.TELEFOON_LINK
EMAIL = D.EMAIL
ADRES = D.ADRES
KVK = D.KVK
# De bronsite noemt geen reactietermijn, dus wordt er geen aantal dagen beloofd
# dat J. Boom nooit heeft toegezegd.
REACTIETIJD = D.REACTIETIJD
...
# Alle beelden komen uit de export van www.jboom.nl. maak_assets.py heeft ze
# naar WebP en AVIF omgezet en in beeldmaten.json vastgelegd welke breedtes er
# van elk bestand bestaan, hoe groot de grootste is en welke alt-tekst erbij
# hoort. Dat bestand wordt hier ingelezen, zodat een nieuwe foto alleen in
# beeldplan.json hoeft te staan en niet ook nog hier.
import json as _json
_maten_bestand = pathlib.Path(__file__).resolve().parent / 'beeldmaten.json'
_beeldmaten = (_json.loads(_maten_bestand.read_text(encoding='utf-8'))
               if _maten_bestand.exists() else {})
for _naam, _r in _beeldmaten.items():
    FOTOS[_naam] = (_naam, _r['breed'], _r['hoog'], min(_r['breedtes']), None,
                    _r['alt'], 'foto', None)


def galerij(prefix):
    """De galerijsleutels met dit voorvoegsel, op volgorde."""
    return sorted(k for k in _beeldmaten if k.startswith(prefix)
...
{sub}
  ...
  <div class="header--container">
    <a href="index.html" class="header--logo" aria-label="Aannemersbedrijf J. Boom, naar de homepage">
      <img class="header--logo-kleur" src="assets/logo/jboom-logo.webp" alt="Aannemersbedrijf J. Boom" width="134" height="40">
      <img class="header--logo-wit" src="assets/logo/jboom-logo-wit.webp" alt="" aria-hidden="true" width="134" height="40">
    </a>
...
def footer():
    """De voet: vier kolommen met de diensten, de projecten en de gegevens.

       De voet van de bronsite heeft alleen een adresblok met het KvK-nummer en
       een link naar de privacyverklaring. Dit template heeft vier kolommen;
       die worden gevuld met de diensten en de projecten, zodat de
       belangrijkste pagina's ook van onderaf bereikbaar zijn. Er staat niets
       in wat de bron niet noemt."""
    diensten = "\n".join(f'            <li><a href="{b}">{t}</a></li>'
                         for b, t, _, _ in ALLE_DIENSTEN)
    projecten = "\n".join(f'            <li><a href="{s["bestand"]}">{s["titel"]}</a></li>'
                          for s in PROJECTSERIES)
    ...
    <div class="footer--widgets">
      <div class="row footer--gap">
        <div class="col-lg-3 col-md-4 col-12 widget">
          <img src="assets/logo/jboom-logo.webp" alt="{D.NAAM_VOLUIT}" width="134" height="40" style="margin-bottom:var(--space-500)">
          <p class="footer--intro">Aanbouw, verbouw, dakkapellen en kunststof kozijnen voor particulieren en Verenigingen van Eigenaren. Een familiebedrijf uit Purmerend, sinds {D.OPRICHTING}.</p>
        </div>
        <div class="col-lg-3 col-md-4 col-12 widget">
          <h2 class="footer__kop">Diensten</h2>
          <ul role="list">
{diensten}
          </ul>
        </div>
        <div class="col-lg-3 col-md-4 col-12 widget">
          ...
          <ul role="list">
{projecten}
            <li><a href="referenties.html">Klanten over J. Boom</a></li>
        ...
        <div class="col-lg-3 col-md-4 col-12 widget">
          ...
          <ul role="list">
            ...
            <li><a href="{D.FACEBOOK}" rel="noopener">Facebook</a></li>
          </ul>
...
ORGANISATIE_LD = f'''{{
  ...
  "name": "{D.NAAM_VOLUIT}",
  "alternateName": "{D.NAAM_KORT}",
  "foundingDate": "{D.OPRICHTING}",
  "url": "{BASIS}/",
  "logo": "{BASIS}/assets/logo/jboom-logo.png",
  "image": "{BASIS}/assets/social/jboom-deelafbeelding.png",
  "email": "{EMAIL}",
...
<link rel="icon" href="assets/favicon/jboom-favicon.ico" sizes="16x16 32x32 48x48" />
<link rel="icon" type="image/png" sizes="32x32" href="assets/favicon/jboom-favicon-32.png" />
<link rel="icon" type="image/png" sizes="192x192" href="assets/favicon/jboom-favicon-192.png" />
<link rel="apple-touch-icon" href="assets/favicon/jboom-apple-touch-icon.png" />

<meta property="og:type" content="website" />
<meta property="og:site_name" content="{D.NAAM_KORT}" />
<meta property="og:locale" content="nl_NL" />
<meta property="og:url" content="{BASIS}/{bestand}" />
<meta property="og:title" content="{titel}" />
<meta property="og:description" content="{omschrijving}" />
<meta property="og:image" content="{BASIS}/assets/social/jboom-deelafbeelding.png" />
<meta property="og:image:type" content="image/png" />
<meta property="og:image:width" content="1200" />
<meta property="og:image:height" content="630" />
<meta property="og:image:alt" content="{D.NAAM_VOLUIT}: aanbouw, verbouw, dakkapellen en kunststof kozijnen" />
<meta name="twitter:card" content="summary_large_image" />
<meta name="twitter:image" content="{BASIS}/assets/social/jboom-deelafbeelding.png" />
...
# ---------------------------------------------------------------------------
#  De band met samenwerkingspartners
# ---------------------------------------------------------------------------
# In het oorspronkelijke template is dit een band met klantlogo's. Die is hier
# twee keer aangepast, en allebei de keren om dezelfde reden: een band moet
# waar zijn.
#
#   1. Het zijn geen opdrachtgevers maar partners. www.jboom.nl noemt op
#      /diensten zes bedrijven bij naam als vaste samenwerkingspartners ("De
#      krachten gebundeld"). Dat is wat hier staat, en de kop van de band zegt
#      dat ook: "Met wie wij samenwerken". Een klantenlijst heeft J. Boom niet
#      gepubliceerd, dus die staat er niet.
#
#   2. Het zijn woordmerken en geen logo's. Er is van geen van deze zes een
#      logobestand, en het logo van een ander bedrijf voeren is niet iets wat
#      je zonder toestemming doet. De band rendert daarom de naam in tekst;
...
def _logoset(verborgen=False):
    """Een reeks merken. De band zet er twee achter elkaar en schuift precies de
       helft op, zodat hij naadloos doorloopt; de tweede reeks is aria-hidden,
       dus een schermlezer hoort de namen een keer. Daarom krijgen logo's in die
       tweede reeks een leeg alt: anders leest hij ze alsnog voor.
...
def ctablok(nr, kop, tekst=None):
    """De afsluitende oproep: één merkvlak over de volle breedte, gecentreerd,
       ...
       De regel eronder wordt per pagina meegegeven; staat er niets, dan valt
       hij terug op de vaste regel uit de copylaag."""
    regel = tekst or C.SLOT_STANDAARD
    return (f'  <section class="cta-slot" id="s{nr}-contact">\n'
            ...
            f'          {knop("Bel " + TELEFOON_WEERGAVE, "tel:" + TELEFOON_LINK, "secondary")}\n'
            '        </div>\n'
            '      </div>\n'
            '    </div>\n'
            '  </section>')
...
#  Referenties
# ---------------------------------------------------------------------------
# Het citatencomponent van het template: een referentie per keer, een foto
# links, het citaat rechts met een streepje boven de naam. site.js laat ze
# automatisch doorlopen met een balkje dat de tijd aangeeft; zonder JavaScript
# staan ze onder elkaar en blijven de pijlen verborgen.
#
# Deze citaten zijn echt. www.jboom.nl heeft een pagina met zeven ondertekende
# referenties, met de naam en de maand die de schrijvers er zelf bij zetten. Ze
# staan in inhoud_jboom.py, ingekort maar niet aangescherpt; de volledige tekst
# staat in jboom/bron/klanten-over-j-boom.txt.
#
# Twee dingen staan er bewust niet in:
#
#   1. Een plaatsnaam bij het citaat. De referenties gaan over verbouwingen bij
#      mensen thuis en er staat geen adres bij. In plaats van de plaats staat er
#      waar het werk over ging (dakopbouw, kunststof kozijnen), en dat volgt uit
#      het citaat zelf.
#   2. Logo's. Dit zijn particulieren; die hebben er geen. Het slot
#      (.quote__logo) blijft daarom leeg.
#
# De foto's zijn foto's van J. Boom uit dezelfde categorie werk als het citaat.
# Er staat niet bij dat het de woning van die klant is, want dat is niet bekend;
# de bijschriften noemen alleen de klant en de maand.
def _referenties():
    """De referenties in de vorm die citaten() verwacht."""
    return [(beeld, onderwerp, datum, citaat, naam, None)
            for _kop, citaat, naam, datum, onderwerp, beeld in D.REFERENTIES]


REFERENTIES = _referenties()


def citaten(nr, ident="referenties", kop="Wat klanten zeggen",
    ...
    dias = []
    for i, (beeld, onderwerp, datum, citaat, wie, logo) in enumerate(items):
        ...
        # Onder het citaat staat waar het werk over ging en wanneer het is
        # geschreven; allebei komt dat uit de referentie zelf.
        herkomst = f'{onderwerp}, {datum}' if datum else onderwerp
        logotag = ''
        if logo:
            logotag = (f'\n        <p class="quote__logo">'
                       f'<img src="{LOGOMAP}{logo}" alt="{wie or onderwerp}" '
                       f'height="32" loading="lazy" decoding="async"></p>')
        dias.append(
            f'      <figure class="quote"{" hidden" if i else ""}>\n'
            f'        <div class="quote__beeld">\n'
            f'          {foto(beeld, maten="(max-width: 767px) 100vw, 33vw")}\n'
            f'        </div>\n'

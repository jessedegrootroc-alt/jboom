> **Let op:** dit document beschreef het sectieskelet van twee sites geleden
> (cases, cursussen, drie diensten). De componenten, de klassenamen en de
> sectievolgorde zijn nog steeds die van dit template en dus nog geldig, maar de
> paginanamen en de voorbeeldteksten bestaan niet meer. De actuele indeling per
> paginatype staat in de docstrings van de bouwscripts in `_generator/`:
> `bouw_jboom_dienst.py` beschrijft het skelet van een dienstpagina,
> `bouw_jboom_projecten.py` dat van een projectserie.
>
> Ook de kleuren en de contrastgetallen in dit document zijn die van vóór de
> merkwissel. De actuele merklaag staat in `brand-style.md`; de metingen die op
> deze site gedaan zijn, staan als commentaar bij de regel die ze onderbouwt
> (`.hero--sluier` in `index.css`, `.cta-slot__hoofd` en `.uitklap__sluier` in
> `styleguide.css`).

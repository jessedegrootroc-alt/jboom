/* ---- Cookiemelding ----
   ...
   zo'n script later alleen hoeft te laden als de bezoeker ja heeft gezegd:
     ...
     window.jboomToestemming            -> { analytisch, marketing }
     window.jboomCookies.open()         -> balk opnieuw tonen
     document.addEventListener('jboom:toestemming', e => e.detail)
*/
(() => {
  const SLEUTEL = 'jboom-cookies-v1';
  ...
  const toepassen = (keuze) => {
    window.jboomToestemming = keuze;
    document.dispatchEvent(new CustomEvent('jboom:toestemming', { detail: keuze }));
  };
  ...
  window.jboomCookies = { open: toon, keuze: () => lees() };

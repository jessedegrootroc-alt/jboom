...
(() => {
  ...
  /* De negen diensten van J. Boom, in de volgorde van het menu, plus twee
     algemene keuzes. Wijzigt de dienstenlijst in _generator/inhoud_jboom.py,
     dan hoort deze lijst mee te wijzigen; ze staan los omdat dit bestand niet
     gegenereerd wordt. */
  const ONDERWERPEN = [
    ['aanbouw', 'Aanbouw of uitbouw'],
    ['verbouw', 'Verbouw'],
    ['nieuwbouw', 'Nieuwbouw'],
    ['dakkapel', 'Dakopbouw of dakkapel'],
    ['renovatie', 'Renovatie'],
    ['deuren', 'Deuren en puien'],
    ['kozijnen', 'Kunststof kozijnen of voorgevel'],
    ['gevelbekleding', 'Gevelbekleding'],
    ['bouwadvies', 'Bouwadvies of bouwtekening'],
    ['offerte', 'Offerteaanvraag'],
    ['overig', 'Overig'],
  ];

  /* TODO-CONTENT: waar moet de inzending naartoe? Zolang dit leeg is gaat er
     niets de deur uit, en zegt het formulier dat ook: het meldt dat het nog
     niet is aangesloten en wijst naar het e-mailadres en het telefoonnummer.
     Zet hieronder het endpoint of de formulierdienst neer; dan verdwijnt die
     melding vanzelf en komt de gewone bevestiging ervoor terug. */
  const ENDPOINT = '';
  ...
  const VELDEN = [
    ...
    { naam: 'bedrijf', label: 'Bedrijf of VvE (optioneel)', type: 'text', autocomplete: 'organization', verplicht: false },
  ...
  const bouw = (houder) => {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        const { ok, gekoppeld } = await submitContactForm(payload);
        if (!ok) throw new Error('verzenden mislukt');

        /* Zolang ENDPOINT leeg is gaat er niets de deur uit. Dan mag hier ook
           niet staan dat het bericht binnen is: dat is het enige op deze site
           wat de bezoeker iets onwaars zou vertellen. Het formulier blijft
           staan met de ingevulde tekst erin, zodat hij te kopiëren is, en de
           twee kanalen die wél werken staan eronder. Zodra het endpoint er is,
           valt deze tak vanzelf weg en komt de gewone bevestiging terug. */
        if (!gekoppeld) {
          melding.textContent = 'Dit formulier is nog niet aangesloten, dus uw bericht is niet verstuurd. '
            + 'Mail uw vraag naar info@jboom.nl of bel 0299 - 43 34 98.';
          melding.className = 'contactformulier__melding is-fout';
          return;
        }

        form.reset();
        /* Geen termijn beloven die J. Boom niet noemt. De bronsite zegt daar
           niets over, dus staat er "zo snel mogelijk" en geen aantal dagen. */
        melding.textContent = 'Bedankt, uw bericht is binnen. Wij nemen zo snel mogelijk contact met u op.';
        melding.className = 'contactformulier__melding is-goed';
      } catch (err) {
        melding.textContent = 'Het versturen lukte niet. Mail ons op info@jboom.nl of bel 0299 - 43 34 98.';
        melding.className = 'contactformulier__melding is-fout';
      } finally {
        knop.disabled = false;
